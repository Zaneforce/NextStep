...
// Debug log to ensure Firebase Firestore is initialized
    ...
    const snapshot = await getDocs(postsQuery);
     ...
     for (const docSnap of snapshot.docs) {
      ...
      const deleteBtn = isAuthor
        ? `<button onclick="deletePost('${postId}')"> <img src="../Assets/trash-alt.png" alt="delete" style="width: 20px; height: 20px; cursor: pointer;" /></button>`
        : "";
      ...
      replySnap.forEach(r => {
        const data = r.data();
        repliesHTML += `<div class="reply"><strong>${data.userName || data.userEmail}</strong>: ${data.content}</div>`;
        replyCount++;
      });

      const replyInputId = `reply_${postId}`;
      const userData = await getUserDataByUid(currentUser.uid);
      const userName = userData ? userData.name : "Anonymous";
      ...
      <div class="post" style="position: relative; cursor:pointer; display: flex; align-items: flex-start;" onclick="showPostModal('${postId}')">
        ...
        <div style="flex:1;">
          ...
          <div class="post-buttons" style="display: flex; gap: 8px; align-items: center;">
            <button class="reply-toggle" onclick="event.stopPropagation(); toggleReplies('${postId}')" style="display: flex; align-items: center; gap: 4px;">
...
window.toggleReplies = function(postId) {
  const replySection = document.getElementById(`replies_${postId}`);
